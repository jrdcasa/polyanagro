...
def parse_arguments():
    ...
    parser.add_argument("--normal", dest="normal", type=int,
                        help="Normal direction to create the slabs. Allowed values"
    ...
    parser.add_argument("--log", dest="log",
                        ...
                        action="store", required=False, default="slab_calcylations_FS.log")
    #
    ...
    if args.normal < 0 or args.normal > 2:
            m = "\t\t\t Normal direction must be 0 for x, 1 for y and 2 for z."
            m += "\t\t\t You give normal = {}".format(args.normal)
            print(m)
            exit()
...
def main_app():
    ...
    objslab = pag.Slab_Statistics(trj, args.normal, dt=trj.dt, stride=args.stride, log=log)
    iswrapped, _ = objslab.normal_coord_max_min()
    objslab.slab_density_calc()
    pass

    #objcalc = pag.Slab_Statistics(trj, dt=trj.dt, stride=args.stride, log=log)
