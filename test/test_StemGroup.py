...
class StemGroupTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.log = pag.init_logger("Output", fileoutput="StemGroup_test/test_stemgroup.log", append=False, inscreen=False)
        cls.log.info("\n\tJob Information\n\t---------------")
    ...
    def test_01_stemgroup_linear(self):
        ...
        stemObj = pag.StemGroup(trj_small_psf)
    ...
    def test_02_stemgroup_branch(self):
        ...
        trj_small_psf = pag.ExtTrajectory([dcd1], topfile=filename_psf, templatefile=tempfile, logger=self.log)
        stemObj = pag.StemGroup(trj_small_psf)
        stemObj.look_for_stems()
